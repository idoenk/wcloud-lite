...
var CanvasView = function CanvasView(opts) {
  this.load(opts, {
    ...
    canvasLayerElement: 'wc-canvas-layer'
  });
...
CanvasView.prototype.draw = function cv_draw(option) {
  if( this.app.DEBUG_LOG )
    console.log('CanvasView::draw', JSON.stringify(option));
  ...
  var that = this;
  $(this.canvasElement)
    .attr('data-target', "#wc-canvas-contextmenu")
    .contextmenu({
      // before: function(e, element, target){
      //   e.preventDefault();
      //   $(that.canvasLayerElement).removeAttr('hidden');
      // },
      onItem: function(e, element){
        if( that.app.DEBUG_LOG )
          console.log('click item, e='+e+'; els='+element);
      }
    });

  $(this.canvasLayerElement).click(function(e){
    e.preventDefault();
...
CanvasView.prototype.handleClick = function cv_handleHover(item,
                                                           dimension, evt) {
  ...
  /*console.log('Click::'+item + ';' + dimension+';' + evt+'; ctrl='+event.ctrlKey+'; shift='+event.shiftKey);
  if( event.shiftKey ){
    console.log('Gonna delete this word: ['+item+']');
