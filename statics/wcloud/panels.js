...
var CPPanelView = function CPPanelView(opts) {
  this.load(opts, {
    ...
    texttypeElement: 'wc-panel-texttype',
    formatorderElement: 'wc-panel-formatorder',
    separatorElement: 'wc-panel-separator'
  });
};
CPPanelView.prototype = new PanelView();
CPPanelView.prototype.submit = function cpv_submit() {
  var el = this.textareaElement;
  var eltype = this.texttypeElement;
  var elorder = this.formatorderElement;
  var elseparator = this.separatorElement;
  ...
  if ( !strtext.length ) {
    ...
    if( elorder.value == 'wordfreq' ){
      for(var i=0; i<lines.length; i++){
        parts = lines[i].split( map_separator[String(elseparator.value)] );
  ...
  if (window.btoa) {
    ...
    submitted = this.dialog.submit(
      (eltype.value == 'wordlist' ? '#base64-list:' : '#base64:')
      + window.btoa(unescape(encodeURIComponent(tmptext))));
  } else {
    submitted = this.dialog.submit(
      (eltype.value == 'wordlist' ? '#list:' : '#text:')
  ...
  if (!submitted) {
    ...
    this.dialog.submit(eltype.value == 'wordlist' ? '#list' : '#text');
  }
};
...
var FilePanelView = function FilePanelView(opts) {
  this.load(opts, {
    ...
    contenttypeElement: 'wc-panel-contenttype',

    formatorderElement: 'wc-panel-file-formatorder',
    separatorElement: 'wc-panel-file-separator'
  });
...
GooglePlusPanelView.prototype.beforeShow = function gppv_beforeShow() {
  ...
  el.src = ASSET_WCLOUD_URI+'go2/src/google-oauth2.js?_=@@timestamp';
  el.onload = el.onerror = (function go2load() {
    ...
    */
    var redirectUri = window.GO2_REDIRECT_URI ||
      document.location.href.replace(/\/(wordcloud)(#.*)?$/i, '/wcloud_go2redirect');
...
var TwitterPanelView = function TwitterPanelView(opts){
  this.load(opts, {
    name: 'twitter1_1',
    element: 'wc-panel-twitter1_1',
    statusElement: 'wc-panel-twitter1_1-status'
  });

  this.stringIds = [
    'twitter-ready',
    'twitter-start-to-login'
  ];

  this.loaded = false;
};
TwitterPanelView.prototype = new PanelView();
TwitterPanelView.prototype.LABEL_LOGGED_IN = 0;
TwitterPanelView.prototype.LABEL_NOT_LOGGED_IN = 1;
