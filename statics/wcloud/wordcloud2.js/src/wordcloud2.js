...
(function(global) {
  ...
  var WordCloud = function WordCloud(canvas, options) {
    ...
    var settings = {
      ...
      weightFactor: .8,
      skewNormalize: null, // buat biar normalizing
      skewnessTreshold: .5, // normalize skewness
      clearCanvas: true,
    ...
    if (typeof settings.shape !== 'function') {
      switch (settings.shape) {
        ...
        */
        ...
        case 'love':
        // http://www.wolframalpha.com/input/?i=polar+plot+
        // 2+-+2+sin+t+%2B+sin+t+sqrt+(abs(cos+t))/(sin+t+%2B+1.4)
          settings.shape = function shapeLove(theta) {
            return 2 - (2 * Math.sin(theta)) + (Math.sin(theta) *
    ...
    if( "object" == typeof settings.color
      ...
      && settings.color.ccolorset.length
      // && settings.custom_themeIndex
      // && settings.custom_themeIndex == settings.theme
    ) {
      getTextColor = (function randomColorGenerator(colors) {
        return (function getRandomColor() {
          return colors[Math.floor(Math.random() * colors.length)];
        });
      })(settings.color.ccolorset);
    }


    if( settings.directFont && !(settings.custom_themeIndex && settings.custom_themeIndex === settings.theme) )
      settings.fontFamily = settings.directFont;
    ...
    var wordcloudmousedown = function wordcloudmousedown(evt){
      var info = getInfoGridFromMouseEvent(evt);
      if (!info)
        return;

      settings.mousedown(info.item, info.dimension, evt);
    };

    var wordcloudcontextmenu = function wordcloudcontextmenu(evt){
      var info = getInfoGridFromMouseEvent(evt);
      if (!info)
        return;

      settings.contextmenu(info.item, info.dimension, evt);
    };
    ...
    /* Normalize weight of frequency, this func is duplicated in fetcher.js */
    var normWeight = function normWeight(iword, weight){
      var M_PI = 3.14159265358979323846; // pi
      var tresh_sbx = -(M_PI / 1.1);
      var radVal = ( ((M_PI/2) + tresh_sbx) * iword) / settings.list.length;
    ...
    var getTextInfo = function getTextInfo(word, weight, rotateDeg) {
      ...
      if( settings.skewNormalize ){
        weight = normWeight(iword, weight);
      }
      ...
      // console.log('word='+word + '; wight='+weight+'; fontSize='+fontSize+'; font='+fctx.font);
    ...
    var drawText = function drawText(gx, gy, info, word, weight,
      ...
      if (getTextColor) {
        ...
        if( "object" == typeof settings.color && settings.color.flag === 'custom-textcolor' ){
          console.log('wordcloud2; settings.color.type='+settings.color.type);
          if(settings.color.type == 'dualtone'){
            if( settings.color.wordlist && settings.color.wordlist.length ){
              if( settings.color.wordlist.indexOf(String(word).toLowerCase()) !== -1 )
                ctx.fillStyle = settings.color.topword;
              else
                ctx.fillStyle = settings.color.otherword;
            }
            else{
              if( settings.color.nword ){
                if( iword <= settings.color.nword )
                  ctx.fillStyle = settings.color.topword;
                else
                  ctx.fillStyle = settings.color.otherword;
              }
          ...
          else if(settings.color.type == 'palette'){



          }
          else{
            if("function" == typeof getTextColor){
              ctx.fillStyle = getTextColor(word, weight, fontSize, distance, theta);
            }
          }
        }
    ...
    var start = function start(canvas) {
      ...
      if (settings.hover || settings.click) {
        ...
        if (settings.mousedown) {
          canvas.addEventListener('mousedown', wordcloudmousedown);
        }

        if (settings.contextmenu) {
          canvas.addEventListener('contextmenu', wordcloudcontextmenu);
        }
        ...
        canvas.addEventListener('wordcloudstart', function stopInteraction() {
          ...
          canvas.removeEventListener('mousedown', wordcloudmousedown);
          canvas.removeEventListener('contextmenu', wordcloudcontextmenu);
          hovered = undefined;
        });
      }
